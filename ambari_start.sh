...
get-ambari-server() {
  ...
  AMBARI_MASTER_NODE=$($KUBE get pod $AMBARI_SERVER_POD -o template --template={{.spec.nodeName}})
  AMBARI_IP=$($KUBE get node $AMBARI_MASTER_NODE -o=template '--template={{(index .status.addresses 2).address}}')
}
